...
public class CharacterMovement : MonoBehaviour, ICharacterController
{
    ...
    public void UpdateVelocity(ref Vector3 currentVelocity, float deltaTime)
    {
        ...
        CheckIsFalling(currentVelocity);
        CheckIsRunning(currentVelocity);
        CheckIsJumping(currentVelocity);
    }

    private void CheckIsFalling(Vector3 currentVelocity)
    {
        if (isGrounded)
        {
            isFalling = false;
    ...
    private void CheckIsRunning(Vector3 currentVelocity)
    {
        if (isGrounded)
        {
            isRunning = currentVelocity.x != 0 && currentVelocity.z != 0;
        } else
        {
            isRunning = false;
        }        
    ...
    private void CheckIsJumping(Vector3 currentVelocity)
    {
        if (currentVelocity.y > 0)
        {
            isJumping = true;
